...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Rivo** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Rivo**  VERSION (left-aligned).
    HeroInline,
}
    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Rivo is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Rivo may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("Rivo"), "full badge: {full:?}");
        assert!(inline.contains("Rivo"), "inline badge: {inline:?}");
